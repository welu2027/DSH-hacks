...
const SponsorsSection = () => {
  return (
    <section id="sponsors" className="bg-[#0d0d0d] py-16 text-white sm:py-24">
      <div className="mx-auto w-full max-w-7xl px-6 sm:px-8">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.3 }} transition={{ duration: 0.6 }}>
          <div className="flex items-center gap-6 mb-10">
            <div className="flex h-20 w-20 shrink-0 items-center justify-center rounded-full bg-[#8007F7] text-4xl font-bold text-white sm:h-24 sm:w-24 sm:text-5xl">3</div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.3 }} transition={{ duration: 0.6, delay: 0.1 }} className="mb-12 max-w-4xl">
          <p className="text-lg sm:text-xl text-white/80 leading-relaxed">
            ...
            <a href="https://dsh-hacks-v1.devpost.com/" target="_blank" rel="noopener noreferrer" className="text-purple-400 hover:text-purple-300 underline transition-colors">Devpost</a>{" "}
              ...
              <motion.div key={tier} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }} transition={{ duration: 0.5 }}>
                ...
                <div className={`grid ${cfg.cols} gap-4`}>
                  {filtered.map((s) => (
                    <div
                      key={s.name}
                      className={`flex items-center justify-center rounded-2xl bg-white/10 border border-white/10 hover:border-[#8007F7]/50 hover:bg-white/15 transition-all ${cfg.py} px-6`}
                    >
